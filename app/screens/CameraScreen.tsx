The CameraScreen component is modified to include user role switching, simplified analyzing animation, and enhanced camera flow with confirmation steps.
```
...
import { Upload } from 'lucide-react-native'; // Added Upload icon
...
const CameraScreen: React.FC<CameraScreenProps> = ({ onNavigate, onBack }) => {
  ...
  const { isDark } = useTheme(); // Added ThemeContext
  ...
  const [isConvertingToPdf, setIsConvertingToPdf] = useState(false); // Add isConvertingToPdf state
